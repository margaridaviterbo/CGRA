 ...
 MyPrism.prototype.initBuffers = function() {
  ...
  for (var i = 0; i <= this.stacks; i++) {
    for (var j = 0; j < this.slices; j++){
      ...
      this.vertices.push(Math.cos((j+1)*angularStep), Math.sin((j+1)*angularStep), i/this.stacks);

      this.normals.push(Math.cos((j+1)*(angularStep/2)), Math.sin((j+1)*(angularStep/2)), 0);
      this.normals.push(Math.cos((j+2)*(angularStep/2)), Math.sin((j+2)*(angularStep/2)), 0);
    }
  }
